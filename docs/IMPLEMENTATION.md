...
- Registration, login, scrypt credentials, hashed sessions, verification tokens, reset tokens, session revocation, and transactional email outbox/worker.
- Owner-scoped websites and content; protected internal super-administration; audit records for privileged mutations.
- Six unpriced subscription products; administrative prices; configurable content caps through the admin API; Basic blog/domain restrictions; server-side limits.
...
- Desktop/mobile layouts and minimum-width browser checks at 280px.
...
| CMS                | Rich text toolbar/tables/embeds, per-page section composition, editable navigation/footer, scheduled publishing, editorial review, revisions/rollback, reusable blocks                                                      |
| Content modules    | Dedicated services, projects, team profiles, testimonials, author profiles, category management, legal-page management                                                                                                      |
| Media              | S3-compatible storage, quota accounting/enforcement, cropping, folders, deletion/reference safety, responsive derivative sets, independent favicon selection                                                                |
...
| Billing            | Automated recurrence, invoices, retry/grace policy, cancellation, proration, upgrade/downgrade impact flow, timed overrides, service billing                                                                                |
...
| Operations         | Full marketing CMS, verification review UI, support replies/attachments/assignment, service quotations/approval workflow, scoped assisted access                                                                            |
...
- Advanced uses an explicit initial cap of 100 content pages and 1,000 articles instead of an undefined “unlimited” allowance. Plan cap overrides are supported by the backend; a cap-editing admin form remains to be added.
