...
export async function TemplatePreview({
  ...
  const industry = templatePreviewIndustry[t.id as keyof typeof templatePreviewIndustry];
  const [row] = await query<{ core_pages: string[] }>(
    "SELECT core_pages FROM industries WHERE id=$1",
    [industry],
  );
  const pages = row?.core_pages?.length ? row.core_pages : ["About", "Services", "Contact"];
  const base = `/templates/${t.id}`;
  const site = previewSite(industry, category, { name: t.business }, { base, pages });
  const articles = category === "corporate" ? sampleArticles(site.kit) : [];
  const products =
    category === "commerce"
      ? (site.kit.sections.find((s) => s.id === "services")?.items || []).slice(0, 4).map((item, i) => ({
          title: item.title,
          image: item.image,
  ...
  if (first === "legal" && second) {
    ...
    body = (
      <article className="rendered-section record-page">
        ...
        <p className="template-legal-note">
          Sample policy. Each Omnyvox website starts with a draft for its industry that
          the owner completes and reviews before publishing.
        </p>
        <div className="rich-content" dangerouslySetInnerHTML={{ __html: safeHtml(policy.body) }} />
      </article>
    );
    ...
    body = article ? (
      <article className="rendered-section record-page record-articles">
        ...
        {article.image && <img className="record-image" src={article.image} alt="" width={900} height={600} />}
        <div className="rich-content" dangerouslySetInnerHTML={{ __html: safeHtml(article.body) }} />
      </article>
    ) : (
      <section className="rendered-section site-insights-page">
        ...
        <h1>Insights</h1>
        <p className="site-page-lead">News, guidance and updates from {t.business}.</p>
        <nav className="site-category-filter" aria-label="Filter articles by category">
          <a href={`${base}/insights`} aria-current={!filter ? "page" : undefined}>
          ...
          {articles.map((a) => (
            <a
              ...
              aria-current={filter === a.category.toLowerCase() ? "page" : undefined}
            >
        ...
        <PreviewArticles
          articles={articles.filter((a) => !filter || a.category.toLowerCase() === filter)}
          base={base}
        />
      </section>
    );
    ...
    body = (
      <>
        <section className="rendered-section preview-page-head">
          <nav className="site-breadcrumbs" aria-label="Breadcrumb">
            <a href={base}>Home</a>
            <span aria-current="page">{corePage}</span>
          </nav>
          <h1>{corePage}</h1>
        </section>
        <RenderSections
          sections={pageSections(site.kit, corePage).map((s) => ({
            ...s,
            ctas: s.ctas?.map((c) => ({ ...c, href: c.href.startsWith("/") ? "/contact" : c.href })),
          }))}
  ...
  const navigation = [
    ...
    ...(products.length ? [{ label: "Shop", href: "/shop", footer: false }] : []),
  ...
  return (
    <main id="main">
      ...
      <p className="template-sample-note">
        Sample content for a fictitious business. Explore the pages using the menu. Your
        website uses your own verified details.
      </p>
      <SiteRenderer
        data={{ ...site.data, sections: homeSections, brand: { ...site.data.brand, navigation } }}
        contact={site.contact}
        legal={site.legal}
        base={base}
        videoEnabled
        footerLinks={articles.length ? [{ label: "Insights", href: "/insights" }] : []}
        insights={articles.length ? <PreviewArticles articles={articles} base={base} /> : undefined}
        after={
...
function PreviewArticles({ articles, base }: { articles: ReturnType<typeof sampleArticles>; base: string }) {
  return (
    <div className="site-article-grid">
      {articles.map((a) => (
        <article className="site-article-card" key={a.slug}>
          <a href={`${base}/insights/${a.slug}`} className="site-article-media" tabIndex={-1} aria-hidden="true">
            {a.image && <img src={a.image} alt="" width={640} height={400} loading="lazy" />}
          </a>
...
function PreviewProducts({ products }: { products: { title: string; image?: string; price: number }[] }) {
  return (
    <div className="product-grid">
      {products.map((p) => (
        <article className="product-card" key={p.title}>
          ...
          <span className="button secondary" aria-disabled="true" title="Shopping works on published stores">
            Sample product
          </span>
        </article>
      ))}
